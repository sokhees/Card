...
- (BOOL)executeUpdate:(NSString*)sql, ... {
    // Khahc 20121023 -- log sql query
    NSLog(@"%@",sql);
    va_list args;
    va_start(args, sql);
    // Khahc 20121023 -- add error and throw if have
    NSError *error = nil;
...
//    BOOL result = [self executeUpdate:sql error:nil withArgumentsInArray:nil orDictionary:nil orVAList:args];
    if(error) {
        NSException *exception = [NSException exceptionWithName: @"Error" reason: [error localizedDescription] userInfo: nil];
        @throw exception;
    }
